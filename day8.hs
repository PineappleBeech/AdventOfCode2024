...
main = do
    ...
    let grid = lines contents
        ...
        antinodes = chars >>=  (map findAntinode . permutate . findChar grid)
    print $ size $ fromList $ filter (inRange gridBounds) antinodes
...
findAntinode ((y1, x1), (y2, x2)) = (2 * y2 - y1, 2 * x2 - x1)
